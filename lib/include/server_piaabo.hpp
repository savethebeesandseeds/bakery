...
static enum MHD_Result session_cookie(
  ...
  const char *read_value = 
    MHD_lookup_connection_value(
      connection, MHD_COOKIE_KIND, "BAKERY_SESSION");
  if(read_value!=NULL){
  ...
  raw_value[64] = '\0';
  snprintf(set_value, sizeof(set_value), "%s=%s","BAKERY_SESSION",raw_value);
  /* setting cookie */
  return MHD_add_response_header(
    response, MHD_HTTP_HEADER_SET_COOKIE, set_value);
  // return MHD_set_connection_value(connection,MHD_COOKIE_KIND,"BAKERY_SESSION",raw_value);
  // return MHD_set_connection_value(connection,MHD_HEADER_KIND,MHD_HTTP_HEADER_SET_COOKIE,value);
  // MHD_set_connection_value(connection,MHD_COOKIE_KIND,"BAKERY_SESSION","waka");
  // MHD_set_connection_value(
  //  connection,MHD_HEADER_KIND,MHD_HTTP_HEADER_SET_COOKIE,"waka");
}
/* respond with HTML */
